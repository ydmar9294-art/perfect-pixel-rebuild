...
const PurchaseReturnsTab: React.FC = () => {
  ...
  const filteredReturns = returns.filter(r => {
    if (searchTerm && r.supplier_name && !r.supplier_name.toLowerCase().includes(searchTerm.toLowerCase())) {
      return false;
    }
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="bg-destructive/10 rounded-2xl p-4 text-center">
        <p className="text-sm text-muted-foreground">إجمالي مرتجعات المشتريات</p>
            ...
            <tbody className="divide-y">
              {filteredReturns.map((ret) => (
                <tr key={ret.id} className="hover:bg-muted/50">
                  <td className="p-4 font-bold">{ret.supplier_name || '-'}</td>
                  <td className="p-4 font-bold text-destructive">{Number(ret.total_amount).toLocaleString('ar-SA')} ر.س</td>
