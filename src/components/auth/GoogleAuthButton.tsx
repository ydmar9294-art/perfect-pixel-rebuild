...
const GoogleAuthButton: React.FC<GoogleAuthButtonProps> = ({ 
  ...
  return (
    <button
      ...
      className={`w-full py-5 bg-white border-2 border-border rounded-2xl font-bold text-base flex items-center justify-center gap-4 active:scale-[0.98] transition-all duration-200 disabled:opacity-50 shadow-lg hover:shadow-xl hover:border-primary/30 ${className}`}
    >
      {loading ? (
        ...
        <>
          ...
          <span className="text-foreground">المتابعة بحساب Google</span>
        </>
      )}
    </button>
  );
